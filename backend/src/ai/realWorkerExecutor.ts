...
const WORKER_ROLES: Record<string, { ro: string; en: string }> = {
  ...
  'meal-plan-generator': {
    ro: 'Esti generatorul de idei de mese adaptate cultural. Creeaza 7 idei simple de mese (nu retete detaliate, nu liste de ingrediente) adaptate la preferintele utilizatorului si contextul sau cultural. Genereaza idei separate pentru mic dejun, pranz si cina. Fiecare idee de masa trebuie sa fie o sugestie scurta si familiara, nu o reteta elaborata. Variaza tipul de bucatarie si contextul cultural. Evita orice aliment raportat ca problematic. CRITIC: campul "ingredients" trebuie sa fie INTOTDEAUNA un array gol []. Pune descrierea mesei in campul "notes". Returneaza breakfast[], lunch[], dinner[] sau meals[].',
    en: 'You are the culturally-adapted meal idea generator. Create 7 simple meal ideas (not detailed recipes, not ingredient lists) adapted to the user\'s preferences and cultural context. Generate separate ideas for breakfast, lunch, and dinner. Each meal idea should be a short, familiar suggestion, not an elaborate recipe. Vary the cuisine type and cultural context. Avoid any food reported as problematic. CRITICAL: the "ingredients" field MUST always be an empty array []. Put the meal description in the "notes" field. Return breakfast[], lunch[], dinner[] or meals[].',
  },
  'recipe-builder': {
