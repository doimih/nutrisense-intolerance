    ...
    <div className="space-y-5">
      <div className="card p-6 space-y-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input className="input-field" type="number" value={retention} onChange={e => setRetention(e.target.value)} min="1" max="365" />
          </div>
          <div>
        ...
        <div className="flex items-center gap-3 pt-2">
          <button onClick={handleSave} className="btn-primary">{saved ? '✓ Saved' : 'Save Settings'}</button>
      ...
      <div className="card p-6 space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Configure your Hetzner Storage Box credentials for remote backups</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="col-span-2 sm:col-span-1">
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">Found in your Hetzner Robot panel under Storage Boxes</p>
          </div>

          <div className="col-span-2 sm:col-span-1">
            <label className="label-text">Protocol</label>
            <select className="input-field" value={hetznerProtocol} onChange={e => setHetznerProtocol(e.target.value)}>
          ...
          <div className="col-span-2">
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">Directory on the Storage Box where backups will be stored</p>
          </div>
        </div>

        {/* Sub-account / SSH key note */}
        <div className="rounded-lg bg-blue-50 border border-blue-200 p-3 flex gap-3">
          ...
          <p className="text-xs text-blue-700">
            For improved security, consider creating a <strong>sub-account</strong> in Hetzner Robot with restricted access, or configure <strong>SSH key authentication</strong> instead of a password.
          </p>
        </div>

        {/* Test result banner */}
        {hetznerTestResult && (
          <div className={`rounded-lg p-3 flex items-center gap-2 text-sm font-medium ${hetznerTestResult === 'success' ? 'bg-green-50 border border-green-200 text-green-700' : 'bg-red-50 border border-red-200 text-red-700'}`}>
      ...
      <div className="card p-6">
        ...
        <div className="space-y-2">
          {mockBackups.map((b) => (
            <div key={b.id} className="flex items-center justify-between py-2.5 px-3 rounded-lg bg-muted/40">
              <div className="flex items-center gap-3">
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${b.status === 'success' ? 'bg-positive' : 'bg-negative'}`} />
                <div>
