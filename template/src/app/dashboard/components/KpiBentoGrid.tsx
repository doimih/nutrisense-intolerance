...
const variantStyles: Record<KpiCardData['variant'], { card: string; icon: string; trend: string }> = {
  default: { card: 'card', icon: 'bg-muted text-muted-foreground', trend: 'text-muted-foreground' },
  positive: { card: 'card border-positive/20 bg-positive-bg/30', icon: 'bg-positive/10 text-positive', trend: 'text-positive' },
  negative: { card: 'card border-negative/20 bg-negative-bg/30', icon: 'bg-negative/10 text-negative', trend: 'text-negative' },
  warning: { card: 'card border-warning/20 bg-warning-bg/40', icon: 'bg-warning/10 text-warning', trend: 'text-warning' },
};
  ...
  return (
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
      {kpiData.map((kpi) => {
        ...
        const content = (
          <div className={`${styles.card} p-5 h-full transition-all duration-150 hover:shadow-elevated ${kpi.href ? 'cursor-pointer' : ''} ${kpi.colSpan || ''}`}>
            ...
            <div className="mb-2">
              <p className="text-3xl font-bold text-foreground tabular-nums leading-none">{kpi.value}</p>
              {kpi.subValue && (
        ...
        return <div key={kpi.id} className={kpi.colSpan || ''}>{content}</div>;
      })}
    </div>
  );
}