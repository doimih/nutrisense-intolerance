      ...
      <div className="mt-4" style={{ height: 240 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={trendData} margin={{ top: 8, right: 8, bottom: 0, left: -16 }}>
            ...
            <ReferenceLine y={7} stroke="var(--negative)" strokeDasharray="4 4" strokeOpacity={0.4} />
            <ReferenceLine y={4} stroke="var(--positive)" strokeDasharray="4 4" strokeOpacity={0.4} />
            <Line
              type="monotone"
              dataKey="intensity"
              stroke="var(--primary)"
              strokeWidth={2.5}
              dot={(props) => {
                const { cx, cy, payload } = props as { cx: number; cy: number; payload: { intensity: number } };
                return (
                  <circle
                    key={`dot-${cx}-${cy}`}
                    cx={cx}
