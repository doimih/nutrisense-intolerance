        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{guidance.recommended_foods.length} items</p>
            </div>
          </div>
          <div className="space-y-3">
            {guidance.recommended_foods.map((food) => (
              <div key={food.id} className="flex items-start gap-3 p-3 rounded-lg bg-positive-bg/40 border border-positive/10">
                <span className={`badge ${categoryColors[food.category] || 'badge-muted'} flex-shrink-0 mt-0.5`}>
                  {food.category}
                </span>
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{guidance.foods_to_avoid.length} items</p>
            </div>
          </div>
          <div className="space-y-3">
            {guidance.foods_to_avoid.map((food) => {
              ...
              return (
                <div key={food.id} className="flex items-start gap-3 p-3 rounded-lg bg-negative-bg/40 border border-negative/10">
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{guidance.sample_meals.length} recipes</p>
            </div>
          </div>
          <div className="space-y-3">
            {guidance.sample_meals.map((meal) => (
              <div key={meal.id} className="p-3 rounded-lg border border-border hover:bg-muted/30 transition-colors">
                ...
                <p className="text-xs text-muted-foreground leading-relaxed mb-2">{meal.description}</p>
