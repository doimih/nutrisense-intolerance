...
function IntensityBadge({ value }: { value: number }) {
  ...
  return (
    <span className={`inline-flex items-center justify-center w-7 h-7 rounded-full text-xs font-bold tabular-nums badge-${variant} ${
      value >= 7 ? 'bg-negative text-white' : value >= 5 ? 'bg-warning text-white' : value >= 3 ? 'bg-accent/20 text-accent' : 'bg-positive/10 text-positive'
    }`}>
      {value}
    </span>
  );
                ...
                >
                  <td className="px-4 py-3 whitespace-nowrap">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {new Date(entry.date + 'T00:00:00').toLocaleDateString('en-GB', { weekday: 'short' })}
                    </p>
                  </td>
                  <td className="px-4 py-3 max-w-[200px]">
                    <p className="text-sm text-foreground truncate">
                      ...
                      {entry.foods_consumed.length > 2 && (
                        <span className="text-muted-foreground"> +{entry.foods_consumed.length - 2}</span>
                      )}
                    </p>
                  </td>
                      ...
                      {entry.symptoms[0] === 'None' ? (
                        ...
                        <>
                          {entry.symptoms.slice(0, 2).map((s) => (
                            <span key={`sym-${entry.id}-${s}`} className="badge-amber text-xs">{s}</span>
                          ))}
                          {entry.symptoms.length > 2 && (
                            <span className="badge-muted text-xs">+{entry.symptoms.length - 2}</span>
                          )}
                        </>
                      )}
                    </div>
