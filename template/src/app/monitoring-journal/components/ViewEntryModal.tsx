...
export default function ViewEntryModal({ entry, onClose }: ViewEntryModalProps) {
  const intensityColor =
    entry.intensity >= 7
      ...
      : entry.intensity >= 3
      ? 'text-accent bg-orange-50 border-orange-200' :'text-positive bg-positive-bg border-positive/20';
      ...
      <div className="space-y-5">
          ...
          <div>
            ...
            <p className="text-xs opacity-70 mt-0.5">
              {entry.intensity >= 7
                ...
                : entry.intensity >= 3
                ? 'Mild — minor symptoms only' :'Minimal — little to no discomfort'}
            </p>
          </div>
        </div>
        ...
        {!entry.notes && (
          <div className="p-3 rounded-lg bg-muted/30 border border-border">
            <p className="text-sm text-muted-foreground italic">No notes recorded for this entry.</p>
          </div>
        )}
      </div>
    </Modal>
