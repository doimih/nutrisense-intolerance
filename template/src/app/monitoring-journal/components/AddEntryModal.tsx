  ...
  return (
    ...
    >
      <form id="add-entry-form" onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
        {/* Date */}
        <div>
          <label htmlFor="entry-date" className="label-text">Date</label>
          <input
            id="entry-date"
            type="date"
        ...
        {/* Foods consumed */}
        <div>
          <label htmlFor="entry-foods" className="label-text">Foods Consumed</label>
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold border-2 transition-all duration-150 active:scale-95 ${
                    isSelected
                      ? symptom === 'None' ?'border-positive bg-positive-bg text-positive' :'border-warning bg-warning-bg text-warning' :'border-border bg-card text-muted-foreground hover:border-primary/30 hover:text-foreground'
                  }`}
                >
                  {symptom}
                </button>
        ...
        <div>
          ...
          <p className="helper-text mb-2">Rate the overall severity of your symptoms (1 = none, 10 = severe)</p>
          <Controller
            name="intensity"
            control={control}
            render={({ field }) => (
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-3">
            {([
              { value: 'good', label: 'Good', emoji: '😊', color: 'border-positive bg-positive-bg text-positive' },
              { value: 'neutral', label: 'Neutral', emoji: '😐', color: 'border-warning bg-warning-bg text-warning' },
              { value: 'bad', label: 'Poor', emoji: '😔', color: 'border-negative bg-negative-bg text-negative' },
            ] as const).map((opt) => (
              <label
                key={`state-${opt.value}`}
        ...
        {/* Notes */}
        <div>
          <label htmlFor="entry-notes" className="label-text">Notes</label>
          <p className="helper-text mb-1.5">Optional — any observations, context, or patterns you noticed</p>
          <textarea
            id="entry-notes"
            rows={3}
