...
function LoginForm() {
  ...
  const {
    ...
    defaultValues: { email: 'sofia@example.com', password: 'NutriSense2026!', rememberMe: false },
  });

  const onSubmit = async (data: LoginFormData) => {
    setLoading(true);
    // BACKEND INTEGRATION: POST /auth/login — send { email, password }, receive JWT token
    await new Promise((r) => setTimeout(r, 1200));
    setLoading(false);
    toast.success('Welcome back, Sofia!');
...
function RegisterForm() {
  ...
  const onSubmit = async (data: RegisterFormData) => {
    setLoading(true);
    // BACKEND INTEGRATION: POST /auth/register — send { name, email, password, diet_preference }, receive JWT token
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
      ...
      <div>
        <label htmlFor="reg-diet" className="label-text">Diet preference</label>
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex flex-col justify-between w-[45%] bg-primary p-12 relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-72 h-72 rounded-full opacity-10" style={{ background: 'radial-gradient(circle, #ffffff 0%, transparent 70%)', transform: 'translate(30%, -30%)' }} />
        <div className="absolute bottom-0 left-0 w-96 h-96 rounded-full opacity-10" style={{ background: 'radial-gradient(circle, #ffffff 0%, transparent 70%)', transform: 'translate(-30%, 30%)' }} />
        ...
        <div className="relative">
          ...
          <h1 className="text-4xl font-bold text-white leading-snug mb-4">
            Know what your body<br />can and cannot handle
          </h1>
          <p className="text-white/80 text-lg leading-relaxed">
            Track food intolerances, log symptoms, and receive AI-powered dietary guidance — personalised to your biology.
          </p>
        </div>
        ...
        <div className="relative space-y-4">
          {[
            ...
            <div key={`feature-${i}`} className="flex items-start gap-3 bg-white/10 rounded-xl px-4 py-3">
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-muted-foreground text-sm">
              {activeTab === 'login' ?'Welcome back — your health data is waiting.' :'Start managing your food intolerances today.'}
            </p>
          </div>
